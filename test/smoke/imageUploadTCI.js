...
describe('Testing Image Uploading Field', function () {
    ...
    it('TCI-004-1 Does not accept PDF', function () {
        browser.url('');
        uploadPDFFile();
        const elem = $('.ant-upload-list-item-info');
        const isExisting = elem.isExisting();
    ...
    it('TCI-006 Image Upload accepts JPG and displays preview', function () {
       browser.url('');
       uploadSmallJPGimage();
       const preview = $(sel.imagePreview).getAttribute('title');
    ...
    it('TCI-008 User can delete JPG preview', function () {
        browser.url('');
        uploadSmallJPGimage();
        const previewDelete = $(sel.imagePreviewDelete).isClickable();
        expect(previewDelete).toEqual(true);
    });
